  ...
  return (
    <div className="space-y-6 max-w-2xl mx-auto">
      ...
      <div className="text-center">
        <h1 className="text-xl sm:text-2xl font-bold text-neutral-900">Send Feedback</h1>
      ...
      <form onSubmit={handleSubmit} className="glass-card space-y-5">
        {/* Feedback Type */}
        <div>
          <label className="block text-sm text-neutral-500 mb-2">Feedback Type</label>
            ...
            {feedbackTypes.map(type => (
              <button
                ...
                className={`p-3 rounded-lg border text-center transition-all duration-300 ${formData.type === type.value
                  ...
                  : 'border-gray-200 bg-white/80 hover:bg-neutral-800/60 hover:border-gray-300'
                  }`}
              >
                <type.icon className={`w-5 h-5 mx-auto mb-1 ${formData.type === type.value ? 'text-blue-400' : type.color
                  }`} />
                <span className={`text-xs ${formData.type === type.value ? 'text-neutral-900 font-medium' : 'text-neutral-500'}`}>
                  {type.value.split(' ')[0]}
                </span>
              </button>
            ))}
        ...
        {/* Subject */}
        <div>
          <label className="block text-sm text-neutral-500 mb-1.5">Subject *</label>
          <input
            type="text"
        ...
        {/* Description */}
        <div>
          <label className="block text-sm text-neutral-500 mb-1.5">Description *</label>
          <textarea
        ...
        {/* Categories */}
        <div>
          <label className="block text-sm text-neutral-500 mb-2">Related to (optional)</label>
            ...
            {categories.map(cat => (
              <button
                ...
                className={`px-3 py-1 rounded-full text-xs transition-all duration-300 border ${formData.categories.includes(cat)
                  ? 'bg-blue-600 border-blue-500 text-neutral-900 shadow-lg shadow-blue-500/20'
                  : 'bg-white/80 border-gray-200 text-neutral-500 hover:text-neutral-900 hover:bg-white/5'
                  }`}
              >
                {cat}
              </button>
        ...
        <div>
          ...
          <div className="flex gap-2 bg-white/80 p-1 rounded-xl border border-gray-200">
                ...
                className={`flex-1 py-2 rounded-lg text-sm transition-all duration-300 ${formData.priority === priority
                  ? priority === 'High'
                    ...
                    : priority === 'Medium'
                      ...
                      : 'bg-blue-500 text-neutral-900 shadow-lg shadow-blue-500/20'
