...
const MOODS = [
  { id: 0, label: 'Love it', emoji: '😍', color: 'hover:bg-slate-100', active: 'bg-blue-700 text-white ring-1 ring-blue-700 ring-offset-1' },
  { id: 1, label: "It's OK", emoji: '😐', color: 'hover:bg-slate-100', active: 'bg-blue-700 text-white ring-1 ring-blue-700 ring-offset-1' },
  { id: 2, label: 'Not great', emoji: '😕', color: 'hover:bg-slate-100', active: 'bg-blue-700 text-white ring-1 ring-blue-700 ring-offset-1' },
  { id: 3, label: 'Help!', emoji: '😰', color: 'hover:bg-slate-100', active: 'bg-blue-700 text-white ring-1 ring-blue-700 ring-offset-1' },
];
      ...
      >
        ...
        {selected.map(tag => (
          <motion.div
            key={tag.id}
            layoutId={`tag-${tag.id}`}
            className="flex items-center gap-1.5 pl-3 pr-1.5 py-1.5 bg-blue-700 text-white rounded-xl shrink-0 text-xs font-semibold"
          >
        ...
        <motion.div layout className="flex flex-wrap gap-2">
          {unselected.map(tag => (
            <motion.button
              ...
              className="flex items-center gap-1.5 px-3 py-2 bg-white border border-slate-200 text-slate-600 rounded-xl text-xs font-medium hover:border-slate-300 hover:shadow-sm hover:text-blue-700 transition-all"
            >
              <motion.span layoutId={`tag-label-${tag.id}`}>{tag.label}</motion.span>
            </motion.button>
          ))}
...
function ComposeModal({ nickname, onClose, onPosted }) {
  ...
  return (
    <>
      ...
      <div
        className="fixed inset-0 z-[200] flex items-end sm:items-center justify-center bg-blue-700/40 backdrop-blur-sm sm:p-4"
        ...
        >
          ...
          <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100 bg-white/50 backdrop-blur-md shrink-0">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-bold text-blue-700">{nickname}</p>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col flex-1 overflow-y-auto custom-scrollbar">
            <div className="p-6 space-y-6 flex-1">
              ...
              <div className="relative">
                <textarea
                  ...
                  className={cn(
                    "w-full text-base border border-slate-200 rounded-2xl px-5 py-4 bg-white text-blue-700 placeholder-slate-400 resize-none outline-none focus:ring-4 focus:ring-slate-100 focus:border-slate-400 transition-all leading-relaxed",
                    textareaClass
                  )}
                />
                <span className={cn(
            ...
            <div className="p-6 pt-4 border-t border-slate-100 bg-slate-50/50 shrink-0 flex items-center justify-between">
              ...
              <button
                ...
                className="flex items-center gap-2 bg-blue-700 hover:bg-blue-600 disabled:bg-slate-200 disabled:text-slate-400 text-white text-sm font-bold px-8 py-3 rounded-xl transition-all shadow-sm active:scale-[0.98]"
              >
      ...
      <div className="flex gap-3 py-3 relative z-10 bg-white">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-baseline gap-2 mb-1 flex-wrap">
            <span className="text-sm font-bold text-blue-700">{reply.nickname}</span>
            {reply.parent_id && (
          ...
          <div className="flex items-center gap-4 mt-2">
            <button onClick={handleLike}
              className={cn(
                ...
                liked ? 'text-blue-700' : 'text-slate-400 hover:text-slate-700'
              )}>
              <ThumbsUp size={12} className={cn("transition-transform group-active/like:scale-75", liked && "fill-blue-700")} /> 
              {localLikes > 0 && localLikes}
            </button>
            <button onClick={() => onReply(reply)}
              className="flex items-center gap-1.5 text-xs font-bold text-slate-400 hover:text-blue-700 transition-colors">
      ...
      <form onSubmit={handleSubmit} className="mt-5 bg-slate-50/50 p-3 sm:p-4 rounded-2xl border border-slate-100">
        {replyingTo && (
          <div className="flex items-center gap-2 mb-3 text-xs font-medium text-slate-600 bg-white border border-slate-200 px-3 py-1.5 rounded-lg w-fit shadow-sm">
            ...
            Replying to <span className="font-bold text-blue-700">{replyingTo.nickname}</span>
          ...
          <div className="flex-1 min-w-0">
            <div className="relative bg-white rounded-xl border border-slate-200 shadow-sm focus-within:ring-2 focus-within:ring-slate-200 focus-within:border-slate-300 transition-all overflow-hidden">
              <textarea ref={inputRef} value={content} onChange={e => setContent(e.target.value.slice(0, REPLY_MAX_CHARS))}
                ...
                className="w-full text-sm px-4 py-3 bg-transparent text-blue-700 placeholder-slate-400 resize-none outline-none block" />
              ...
              <div className="flex items-center justify-between px-3 py-2 bg-slate-50 border-t border-slate-100">
                ...
                <button type="submit" disabled={posting || !content.trim()}
                  className="flex items-center gap-1.5 bg-blue-700 hover:bg-blue-600 disabled:opacity-30 disabled:hover:bg-blue-700 text-white text-xs font-bold px-4 py-1.5 rounded-lg transition-all active:scale-[0.98]">
        ...
        <div className="flex gap-4">
          ...
          <div className="flex-1 min-w-0 pt-0.5">
            <div className="flex items-center justify-between gap-4 mb-2">
              <div className="flex items-center gap-2 flex-wrap">
                <span className="text-sm font-bold text-blue-700">{post.nickname}</span>
            ...
            return (
              <button key={field} onClick={() => handleReact(field)}
                className={cn(
                  ...
                  hasReacted 
                    ? active 
                    : 'bg-white border-slate-200 text-slate-500 hover:bg-slate-50 hover:text-blue-700'
                )}>
      ...
      <header className="fixed top-0 w-full bg-white/70 backdrop-blur-xl z-[100] border-b border-slate-200/60 shadow-[0_1px_2px_rgba(0,0,0,0.02)]">
        <div className="max-w-6xl mx-auto px-4 lg:px-6 h-16 flex items-center gap-4">
          <Link to="/" className="flex items-center gap-2.5 mr-2 group">
            <div className="w-8 h-8 rounded-xl bg-blue-700 flex items-center justify-center shadow-sm group-hover:scale-105 transition-transform">
              <Sparkles className="text-white size-4" />
            </div>
            <span className="font-extrabold text-xl tracking-tight text-blue-700 hidden sm:block">
              Saveetha<span className="text-slate-400 font-medium">AM</span>
            </span>
          </Link>
          
          <div className="h-5 w-px bg-slate-200 hidden sm:block mx-1"></div>
          <span className="font-bold text-blue-600 hidden sm:block text-sm tracking-wide">Community</span>
          ...
          <div className="ml-auto flex items-center gap-3 sm:gap-4">
            ...
            {editingNick ? (
              <div className="flex items-center gap-2 bg-slate-100 p-1 rounded-xl">
                <input value={nickDraft} onChange={e => setNickDraft(e.target.value.slice(0, NICK_MAX_CHARS))}
                  ...
                  autoFocus className="text-sm font-semibold bg-transparent w-24 sm:w-32 outline-none px-2 text-blue-700 placeholder-slate-400" placeholder="Display name" />
                <button onClick={() => { saveNickname(nickDraft); setNickname(nickDraft); setEditingNick(false); }}
                  className="text-xs bg-blue-700 text-white px-3 py-1.5 rounded-lg font-bold shadow-sm">Save</button>
              </div>
            ) : (
              <button onClick={() => { setNickDraft(nickname); setEditingNick(true); }}
                ...
                <Avatar name={nickname} size="sm" />
                <span className="font-bold text-sm text-slate-700 group-hover:text-blue-700 hidden sm:block">{nickname}</span>
              </button>
            )}
            ...
            <button onClick={() => { if (isBanned()) { setShowBan(true); return; } setShowCompose(true); }}
              className="flex items-center gap-1.5 bg-blue-700 hover:bg-blue-600 text-white text-sm font-bold px-4 py-2 sm:px-5 sm:py-2.5 rounded-xl transition-all shadow-sm active:scale-[0.98]">
            ...
            <div className="sticky top-24 space-y-6">
              ...
              <div className="bg-white border border-slate-200/80 rounded-[1.5rem] p-5 shadow-sm">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    <h2 className="text-base font-bold text-blue-700 tracking-tight">Discussion Board</h2>
              ...
              {/* Guidelines */}
              <div className="bg-blue-700 rounded-[1.5rem] p-5 shadow-sm text-white">
          ...
          <div className="flex-1 min-w-0 order-1 lg:order-2">
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 mb-6">
              ...
              <div className="relative flex-1 group">
                <Search size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-blue-700 transition-colors" />
                <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search discussions..."
                  className="w-full pl-11 pr-4 py-3 text-sm font-medium bg-white border border-slate-200/80 rounded-full outline-none focus:ring-2 focus:ring-blue-700 focus:border-transparent transition-all shadow-sm" />
              </div>

              {/* Mobile Category Select */}
              <div className="relative lg:hidden">
                <select value={categoryFilter} onChange={e => setCategoryFilter(e.target.value)}
                  className="w-full text-sm bg-white border border-slate-200/80 rounded-full pl-4 pr-10 py-3 outline-none focus:ring-2 focus:ring-blue-700 appearance-none font-bold text-slate-700 shadow-sm">
              ...
              <div className="flex bg-slate-200/50 p-1 rounded-full shrink-0 self-start sm:self-auto w-full sm:w-auto">
                <button onClick={() => setSort('latest')}
                  className={cn(
                    ...
                    sort === 'latest' ? "bg-white text-blue-700 shadow-sm" : "text-slate-500 hover:text-slate-700"
                  )}>
                ...
                <button onClick={() => setSort('hot')}
                  className={cn(
                    "flex-1 sm:flex-none px-5 py-2 flex items-center justify-center gap-1.5 text-sm font-bold rounded-full transition-all duration-200",
                    sort === 'hot' ? "bg-white text-blue-700 shadow-sm" : "text-slate-500 hover:text-slate-700"
                  )}>
                  <Zap size={14} /> Hot
                </button>
              </div>
              ...
              {liveCount > 0 && (
                <motion.div initial={{ y: -10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -10, opacity: 0 }} className="mb-6 flex justify-center">
                  <button onClick={() => { setLiveCount(0); setPage(0); fetchPosts(0, categoryFilter, search, sort, true); }}
                    className="flex items-center gap-2 bg-blue-700 text-white text-sm font-bold px-6 py-2.5 rounded-full transition-transform hover:scale-105 shadow-md">
            ...
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center py-20 px-4 text-center bg-white border border-slate-200/80 rounded-[2rem] shadow-sm">
                ...
                <h3 className="text-xl font-bold text-blue-700 mb-2">No discussions found</h3>
                ...
                <button onClick={() => setShowCompose(true)}
                  className="flex items-center gap-2 bg-blue-700 text-white px-6 py-3 rounded-xl text-sm font-bold hover:bg-blue-600 transition-all shadow-sm active:scale-95">
            ...
            {!loading && hasMore && posts.length > 0 && (
              <button onClick={() => { const next = page + 1; setPage(next); fetchPosts(next, categoryFilter, search, sort, false); }}
                ...
                className="w-full mt-8 flex items-center justify-center gap-2 text-sm font-bold text-blue-700 bg-white border border-slate-200 hover:border-slate-300 rounded-xl py-4 transition-all shadow-sm disabled:opacity-50">
