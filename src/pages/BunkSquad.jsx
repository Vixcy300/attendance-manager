    ...
    return (
        <div className="p-4 md:p-6 lg:p-8 max-w-5xl mx-auto space-y-6">
            <div className="flex items-center gap-3 mb-8">
                ...
                <div>
                    <h1 className="text-2xl font-bold text-white">Bunk Squad Sync</h1>
                    <p className="text-neutral-400">Sync attendance with friends before skipping together</p>
                </div>
            </div>
            ...
            {!inSquad ? (
                <div className="grid md:grid-cols-2 gap-6">
                    {/* Create Squad */}
                    <div className="glass-card p-8 border-neutral-800 text-center space-y-6">
                        <div className="w-16 h-16 bg-pink-500/10 rounded-full flex items-center justify-center mx-auto mb-4">
                            <Plus className="w-8 h-8 text-pink-400" />
                        </div>
                        <h2 className="text-xl font-bold text-white">Create a Squad</h2>
                        <p className="text-neutral-400 text-sm">Start a new bunk group and invite your friends to sync your schedules.</p>
                        <button 
                            onClick={generateCode}
                            className="w-full bg-pink-600 hover:bg-pink-500 text-white py-3 rounded-xl font-bold transition-colors shadow-lg shadow-pink-500/20"
                        >
                            Generate Squad Code
                        </button>
                    </div>
                    ...
                    <div className="glass-card p-8 border-neutral-800 text-center space-y-6">
                        ...
                        <p className="text-neutral-400 text-sm">Have an invite code from a friend? Enter it below to link up.</p>
                        <form onSubmit={joinSquad} className="space-y-4">
                            <input 
                                ...
                                className="w-full bg-black border border-neutral-800 rounded-xl px-4 py-3 text-center text-white focus:outline-none focus:border-blue-500 uppercase tracking-widest font-mono"
                            />
                            <button 
                                type="submit"
                                disabled={syncing || joinCode.length < 5}
                                className="w-full bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white py-3 rounded-xl font-bold transition-colors shadow-lg shadow-blue-500/20 flex justify-center items-center gap-2"
                            >
                ...
                <div className="space-y-6 animate-in fade-in zoom-in-95 duration-500">
                    ...
                    <div className="glass-card p-6 border-pink-500/30 bg-pink-500/5 flex flex-col md:flex-row items-center justify-between gap-4">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h2 className="text-xl font-bold text-white">Active Squad</h2>
                                <p className="text-neutral-400 text-sm">3 members connected</p>
                            </div>
                        </div>
                        <div className="flex items-center gap-3 bg-black px-4 py-2 rounded-lg border border-neutral-800 cursor-pointer hover:border-pink-500/50 transition-colors" onClick={() => {navigator.clipboard.writeText(squadCode); toast.success('Code copied!')}}>
                    ...
                    <div className="grid md:grid-cols-3 gap-4">
                        {squadMembers.map(member => (
                            <div key={member.id} className="glass-card p-5 border-neutral-800 relative overflow-hidden">
                                {member.isMe && <div className="absolute top-0 left-0 w-full h-1 bg-blue-500"></div>}
                                <div className="flex items-center gap-4 mb-4">
                                    <div className="text-3xl bg-neutral-900 w-12 h-12 flex items-center justify-center rounded-full">
                                        {member.avatar}
                                    </div>
                                    <div>
                    ...
                    <div className="mt-8">
                        <button 
                            ...
                            disabled={syncing}
                            className="w-full group relative overflow-hidden bg-gradient-to-r from-pink-600 to-purple-600 hover:from-pink-500 hover:to-purple-500 text-white p-6 rounded-2xl font-black text-xl transition-all shadow-[0_0_40px_rgba(219,39,119,0.3)] disabled:opacity-50"
                        >
