...
const WhatsNew = () => {
  ...
  const features = [
    { icon: Palette, title: 'Professional Blue', description: 'New premium color theme', color: 'from-blue-500 to-indigo-600', bg: 'bg-blue-600' },
    { icon: Award, title: 'New Exam Schedules', description: 'Detailed plans for 2024 & 2025', color: 'from-amber-500 to-orange-500', bg: 'bg-amber-500' },
    ...
    { icon: Star, title: 'Mobile Ready', description: 'Perfect on any device', color: 'from-blue-400 to-indigo-400', bg: 'bg-indigo-500' },
  ];
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/90 backdrop-blur-sm">
      <div
        ref={modalRef}
        className="bg-gray-50 border border-gray-200 rounded-3xl w-full max-w-md overflow-hidden shadow-2xl"
      >
        {/* Animated Header */}
          ...
          <div className="relative flex items-center gap-4">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-xl font-bold text-neutral-900">What's New</h2>
                <span className="text-xs bg-white/20 px-2 py-0.5 rounded-full text-neutral-900 font-medium">v4.0</span>
              </div>
            ...
            {features.map((feature, index) => (
              ...
              >
                <div className={`p-2.5 rounded-xl ${feature.bg} transition-transform group-hover:scale-110`}>
                  <feature.icon className="text-neutral-900" size={18} />
                </div>
                ...
                <ChevronRight
                  size={16}
                  className={`text-neutral-600 transition-transform ${activeIndex === index ? 'translate-x-1 text-neutral-900' : ''}`}
                />
              </div>
            ))}
          </div>
        ...
        <div className="p-4 pt-2 border-t border-neutral-800/50">
          <button
            onClick={handleClose}
            className="w-full py-3.5 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-neutral-900 font-semibold rounded-xl transition-all hover:scale-[1.02] active:scale-[0.98] flex items-center justify-center gap-2"
          >
            <Rocket size={18} />
            Let's Explore!
          </button>
